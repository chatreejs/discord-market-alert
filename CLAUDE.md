...
- **`MarketData`** (`src/services/market-data/`) — abstract base with two concrete implementations, split by data source per market: `MarketDataApi` fetches NASDAQ from `api.nasdaq.com` (and throws for SET); `MarketDataScraper` scrapes SET index data from `settrade.com` using Puppeteer + XPath (and throws for NASDAQ). `DiscordBot` calls the right one for each market.
